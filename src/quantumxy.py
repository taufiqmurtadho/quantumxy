...
def method1_hamiltonian(h,J,N):
    ...
    for sequence in bitsequence:
        if sequence[N-1]!=sequence[0]:                                  #periodic boundary condition
            ...
            mat[bitsequence.index(sequence), bitsequence.index(newseq)]+= -J
        for i in range(N-1):
...
def method2_hamiltonian(h,J,N):
    ...
    for i in range(N):
        if i == 0:
            hopping = -J*np.kron(np.kron(pauli_plus, pauli_minus),np.identity(2**(N-2)))
            mat+= -h*np.kron(pauliz, np.identity(2**(N-1)))
            mat+=hopping+np.conj(hopping.T)
        elif i == N-1:                                                      #periodic boundary condition
            hopping = -J*np.kron(np.kron(pauli_minus, np.identity(2**(N-2))), pauli_plus)
            mat+= -h*np.kron(np.identity(2**(N-1)), pauliz)
            mat+= hopping+np.conj(hopping.T)
        else:
            hopping = -J*np.kron(np.kron(np.identity(2**i), np.kron(pauli_plus, pauli_minus)), np.identity(2**(N-i-2)))
